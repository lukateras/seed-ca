...
var p224 = &PrivateKey{
	&ecdsa.PrivateKey{
		PublicKey: ecdsa.PublicKey{
			Curve: elliptic.P224(),
			X:     hexInt("00CF08DA5AD719E42707FA431292DEA11244D64FC51610D94B130D6C"),
			Y:     hexInt("EEAB6F3DEBE455E3DBF85416F7030CBD94F34F2D6F232C69F3C1385A"),
		},
		D: hexInt("F220266E1105BFE3083E03EC7A3A654651F45E37167E88600BF257C1"),
	},
}
...
var p384 = &PrivateKey{
	&ecdsa.PrivateKey{
		PublicKey: ecdsa.PublicKey{
			Curve: elliptic.P384(),
			X:     hexInt("EC3A4E415B4E19A4568618029F427FA5DA9A8BC4AE92E02E06AAE5286B300C64DEF8F0EA9055866064A254515480BC13"),
			Y:     hexInt("8015D9B72D7D57244EA8EF9AC0C621896708A59367F9DFB9F54CA84B3F1C9DB1288B231C3AE0D4FE7344FD2533264720"),
		},
		D: hexInt("6B9D3DAD2E1B8C1C05B19875B6659F4DE23C3B667BF297BA9AA47740787137D896D5724E4C70A825F872C9EA60D2EDF5"),
	},
}
...
func testFixture(f *fixture, t *testing.T) {
	sig1 := &Signature{hexInt(f.r), hexInt(f.s)}
	sig2 := Sign(f.key, []byte(f.msg), f.h)

	if !sig1.Equal(sig2) {
		curveName := f.key.PublicKey.Curve.Params().Name
		t.Errorf(`
Curve %s, hash function %s, message "%s"
  expected:
    R = %X
    S = %X
  got:
    R = %X
    S = %X`, curveName, hashNames[f.h], f.msg, sig1.R, sig1.S, sig2.R, sig2.S)
	}
}
